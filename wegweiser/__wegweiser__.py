...
def parse_options():
	parser = argparse.ArgumentParser(description='A python wikipedia geo spot scraper.')
...
def get_spots(wikiobj, language):
	...
	for obj in wikiobj:
		if wiki_url_pattern.match(obj):
			...
			# search term
			try:
				spot = Spot.from_search(Search(obj,
					language=language))
...
def generate_map(spots, map=None, size=None, type=None, path=False,
		region=False, filename=None):
	labels = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'
	size_x, size_y = size.split('x')
	geomap = Map(size_x=int(size_x), size_y=int(size_y), maptype=type,
	...
	if region is True:
		for index, spot in enumerate(spots):
			geomap.add_path_latlon(str(spot.latitude), str(spot.longitude))
			if index == max_index:
				geomap.add_path_latlon(str(spots[0].latitude),
					str(spots[0].longitude))
	else:
		for index, spot in enumerate(spots):
			geomap.add_marker(spot, label=labels[index])
	if filename is not None:
...
def run(args):
	...
	# map
	if hasattr(args, 'map'):
		generate_map(spots, map=args.map, size=args.size, type=args.type,
