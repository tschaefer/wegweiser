...
class CoreTest(BaseTest):
	...
	def test_Search(self):
		...
		self.assertIs(search, Search)
		self.assertRaises(ValueError, lambda: Search('New York',
			language='it'))
		# limit
		...
		self.assertDictKeysEqual(search.results[0],
			{
				'title': 'title',
				'url': 'url',
			}
		)
		self.failUnlessRaises(AttributeError, setattr, search, "results",
			"set not allowed")
		# term
		self.assertIs(search.term, str)
		self.assertStrEqual(search.term, 'New York')
		self.failUnlessRaises(AttributeError, setattr, search, "term",
			"set not allowed")
		# language
		self.assertIs(search.language, str)
		self.assertStrEqual(search.language, 'en')
		self.failUnlessRaises(AttributeError, setattr, search, "language",
			"set not allowed")
		# limit
		self.assertIs(search.limit, int)
		self.assertEqual(search.limit, 5)
		self.failUnlessRaises(AttributeError, setattr, search, "limit",
			"set not allowed")
	...
	def test_Scrape(self):
		...
		self.failUnlessRaises(AttributeError, setattr, scrape, "url",
			"set not allowed")
		# title
		self.assertIs(scrape.title, str)
		self.failUnlessRaises(AttributeError, setattr, scrape, "title",
			"set not allowed")
		# latitude
		self.assertIs(scrape.latitude, float)
		self.failUnlessRaises(AttributeError, setattr, scrape, "latitude",
			"set not allowed")
		# longitude
		self.assertIs(scrape.longitude, float)
		self.failUnlessRaises(AttributeError, setattr, scrape, "longitude",
			"set not allowed")
		# elevation
		self.assertIs(scrape.elevation, float)
		self.failUnlessRaises(AttributeError, setattr, scrape, "elevation",
			"set not allowed")
	...
	def test_Spot(self):
		...
		spot = Spot(title='New York', latitude=-74.0058333333,
		...
		self.failUnlessRaises(AttributeError, setattr, spot, "url",
			"set not allowed")
		# title
		self.assertIs(spot.title, str)
		self.failUnlessRaises(AttributeError, setattr, spot, "title",
			"set not allowed")
		# latitude
		self.assertIs(spot.latitude, float)
		self.failUnlessRaises(AttributeError, setattr, spot, "latitude",
			"set not allowed")
		# longitude
		self.assertIs(spot.longitude, float)
		self.failUnlessRaises(AttributeError, setattr, spot, "longitude",
			"set not allowed")
		# elevation
		self.assertIs(spot.elevation, float)
		self.failUnlessRaises(AttributeError, setattr, spot, "elevation",
			"set not allowed")
...
class MarkupTest(BaseTest):
	...
	def test_Markup(self):
		...
		markup = Markup(title='NewYorkCity',
		...
		markup.add_spot(Spot.from_search(Search('New York City',
			language='en')))
		...
		self.failUnlessRaises(AttributeError, setattr, markup, "title",
			"set not allowed")
		# spots
		self.assertIs(markup.spots, list)
		self.failUnlessRaises(AttributeError, setattr, markup, "spots",
			"set not allowed")
		# filename
...
class MapTest(BaseTest):
	...
	def test_Map(self):
		...
		# add_marker
		self.assertRaises(ValueError, lambda:
			map.add_marker(Search('New York City')))
		...
		self.failUnlessRaises(AttributeError, setattr, map, "spots",
			"set not allowed")
		# filename
